...
class Camera2Helper(val mActivity: Activity, private val mTextureView: TextureView) {
    ...
    companion object {
        const val PREVIEW_WIDTH = 960     // 1000 750                         //预览的宽度
        const val PREVIEW_HEIGHT = 720                                       //预览的高度
        const val SAVE_WIDTH = 200                                            //保存图片的宽度
        const val SAVE_HEIGHT = 200                                          //保存图片的高度
    }
...
//            savePicSize.toList())
    ...
    private fun transformImage(width: Int, height: Int) {
        ...
        val previewRectF = RectF(0f, 0f, PREVIEW_HEIGHT.toFloat(), PREVIEW_WIDTH.toFloat())
        val centerX = textureRectF.centerX()
        val centery = textureRectF.centerY()

        previewRectF.offset(centerX - previewRectF.centerX(), centery - previewRectF.centerY())
        matrix.setRectToRect(textureRectF, previewRectF, Matrix.ScaleToFit.FILL)
        val scale = Math.max(width.toFloat() / mPreviewSize.getWidth(), height.toFloat() / mPreviewSize.getHeight())
