package com.quyueke.ok.kids.utils.camera
...
import com.acadsoc.aiclass.utils.camera.AutoFitTextureView
import com.acadsoc.aiclass.utils.camera.BitmapUtils
...
class Camera2HelperFace(val mActivity: Activity, private val mTextureView: AutoFitTextureView) {
    ...
    private fun initCameraInfo() {
        ...
        Timber.i("预览最优尺寸 ：${mPreviewSize.width} * ${mPreviewSize.height}, 比例  ${mPreviewSize.width.toFloat() / mPreviewSize.height}")
        Timber.i("保存图片最优尺寸 ：${mSavePicSize.width} * ${mSavePicSize.height}, 比例  ${mSavePicSize.width.toFloat() / mSavePicSize.height}")
    ...
    private fun initFaceDetect() {
        ...
        Timber.i("成像区域  ${activeArraySizeRect.width()}  ${activeArraySizeRect.height()} 比例: ${activeArraySizeRect.width().toFloat() / activeArraySizeRect.height()}")
        Timber.i("预览区域  ${mPreviewSize.width}  ${mPreviewSize.height} 比例 ${mPreviewSize.width.toFloat() / mPreviewSize.height}")

        for (mode in faceDetectModes) {
            Timber.i("支持的人脸检测模式 $mode")
        }
        Timber.i("同时检测到人脸的数量 $faceDetectCount")
    }
    ...
    private fun handleFaces(result: TotalCaptureResult) {
        ...
        for (face in faces) {
            ...
            Timber.i("原始人脸位置: ${bounds.width()} * ${bounds.height()}   ${bounds.left} ${bounds.top} ${bounds.right} ${bounds.bottom}   分数: ${face.score}")
            Timber.i("转换后人脸位置: ${resultFaceRect.width()} * ${resultFaceRect.height()}   ${resultFaceRect.left} ${resultFaceRect.top} ${resultFaceRect.right} ${resultFaceRect.bottom}   分数: ${face.score}")
        }

        mActivity.runOnUiThread {
            mFaceDetectListener?.onFaceDetect(faces, mFacesRect)
        }
        Timber.i("onCaptureCompleted  检测到 ${faces.size} 张人脸")
    }
    ...
    private fun getBestSize(targetWidth: Int, targetHeight: Int, maxWidth: Int, maxHeight: Int, sizeList: List<Size>): Size {
        ...
        for (size in sizeList) {
            ...
            Timber.i("系统支持的尺寸: ${size.width} * ${size.height} ,  比例 ：${size.width.toFloat() / size.height}")
        }

        Timber.i("最大尺寸 ：$maxWidth * $maxHeight, 比例 ：${targetWidth.toFloat() / targetHeight}")
        Timber.i("目标尺寸 ：$targetWidth * $targetHeight, 比例 ：${targetWidth.toFloat() / targetHeight}")
    ...
    private fun exchangeWidthAndHeight(displayRotation: Int, sensorOrientation: Int): Boolean {
        ...
        when (displayRotation) {
            ...
            else -> Timber.i("Display rotation is invalid: $displayRotation")
        }

        Timber.i("屏幕方向  $displayRotation")
        Timber.i("相机方向  $sensorOrientation")
        return exchange
    }
